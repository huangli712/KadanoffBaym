...
function exact_leftmultiply_tstp(β::F64, tmax::F64, G::ℱ{T}) where {T}
    ...
    for m = 1:ntau
        ...
        for n = 1:ntime
            t1 = (n - 1) * δt
			lmix[1,1] = (-0.17677669529663687+0.2928932188134524im)*exp(2.0*(β-τ+im*t1)) + (0.1767766952966371+1.707106781186548im)*exp(-2.0*im*t1 + τ*2.0)
			lmix[1,2] = (0.7071067811865475+0.4267766952966369im)*exp(2.0*(β-τ+im*t1)) + (-0.7071067811865475+0.07322330470336313im)*exp(-2.0*im*t1 + τ*2.0)
			lmix[2,1] = (-1.0606601717798212+0.07322330470336319im)*exp(2.0*(β-τ+im*t1)) + (1.0606601717798216+0.426776695296637im)*exp(-2.0*im*t1 + τ*2.0)
			lmix[2,2] = (0.17677669529663675+2.5606601717798214im)*exp(2.0*(β-τ+im*t1)) + (-0.1767766952966369+0.4393398282201787im)*exp(-2.0*im*t1 + τ*2.0)
