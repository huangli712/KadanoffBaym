...
function exact_leftmultiply_tstp(β::F64, tmax::F64, G::ℱ{T}) where {T}
    ...
    for m = 1:ntau
        tau = (m - 1) * δτ
		mat[1,1]=(-1.7071067811865475+0.17677669529663675im)*exp(-2*tau)*exp(β*2.0) + (-0.2928932188134524-0.17677669529663687im)*exp(2.0*tau)
		mat[1,2]=(-0.07322330470336319-0.7071067811865475im)*exp(-2*tau)*exp(β*2.0) + (-0.4267766952966368+0.7071067811865475im)*exp(2.0*tau)
		mat[2,1]=(-0.4267766952966371+1.0606601717798207im)*exp(-2*tau)*exp(β*2.0) + (-0.0732233047033631-1.0606601717798212im)*exp(2.0*tau)
		mat[2,2]=(-0.43933982822017864-0.17677669529663687im)*exp(-2*tau)*exp(β*2.0) + (-2.560660171779821+0.17677669529663687im)*exp(2.0*tau)
        @. G.mat[m] = mat / ( 1.0 + exp(2.0*β) )

        for n = 1:ntime
            t1 = (n - 1) * δt
			lmix[1,1] = (-0.17677669529663687+0.2928932188134524im)*exp(2.0*(β-tau+im*t1)) + (0.1767766952966371+1.707106781186548im)*exp(-2.0*im*t1 + tau*2.0)
			lmix[1,2] = (0.7071067811865475+0.4267766952966369im)*exp(2.0*(β-tau+im*t1)) + (-0.7071067811865475+0.07322330470336313im)*exp(-2.0*im*t1 + tau*2.0)
			lmix[2,1] = (-1.0606601717798212+0.07322330470336319im)*exp(2.0*(β-tau+im*t1)) + (1.0606601717798216+0.426776695296637im)*exp(-2.0*im*t1 + tau*2.0)
			lmix[2,2] = (0.17677669529663675+2.5606601717798214im)*exp(2.0*(β-tau+im*t1)) + (-0.1767766952966369+0.4393398282201787im)*exp(-2.0*im*t1 + tau*2.0)
			@. G.lmix[n,m] = cos(t1) * lmix / ( 1.0 + exp(2.0*β) )
        end
    end
